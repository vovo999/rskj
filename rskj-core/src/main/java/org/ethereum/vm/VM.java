...
public class VM {
    ...
    protected void doCODESIZE() {
        ...
        if (op == OpCode.CODESIZE) {
            ...
            ActivationConfig.ForBlock activationConfig = program.getActivationConfig();
            if (activationConfig.isActive(RSKIP90)) {
                PrecompiledContracts.PrecompiledContract precompiledContract = precompiledContracts.getContractForAddress(activationConfig, address);
    ...
    protected void doLOG(){
        if (program.isStaticCall() && program.getActivationConfig().isActive(RSKIP91)) {
    ...
    protected void doCALL(){
        ...
        ActivationConfig.ForBlock config = program.getActivationConfig();

        if (config.isActive(RSKIP103)) {
    ...
    private void callToAddress(DataWord codeAddress, MessageCall msg) {
        ActivationConfig.ForBlock activationConfig = program.getActivationConfig();
        PrecompiledContracts.PrecompiledContract contract = precompiledContracts.getContractForAddress(activationConfig, codeAddress);
